...
<div class="row">
  ...
  $inicio = ($pag * $maximo) - $maximo;
   ...
   if( $gender == "male"){
      ...
      $seleciona_produtos = mysql_query("SELECT * FROM catech WHERE categoria=1  ORDER BY id DESC LIMIT $inicio, $maximo"
);  $seguidor = mysql_query("SELECT * FROM seguidores WHERE seguidor='$id'") ;
    $query_seguidor = mysql_fetch_array($seguidor);}
	 if( $gender == "female"){
 
      $seleciona_produtos = mysql_query("SELECT * FROM catech WHERE categoria=2  ORDER BY id DESC LIMIT $inicio, $maximo"
);  $seguidor = mysql_query("SELECT * FROM seguidores WHERE seguidor='$id'") ;
    $query_seguidor = mysql_fetch_array($seguidor);}
