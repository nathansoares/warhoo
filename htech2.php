...
<!-- /.row -->
  ...
  $inicio = ($pag * $maximo) - $maximo;
    ...
    if( ($gender == "male")){
      $seleciona_produtos = mysql_query("SELECT * FROM tech WHERE categoria=1 && type=1 ORDER BY id DESC  LIMIT $inicio, $maximo"
); 
$seleciona_seguindo = mysql_query("SELECT * FROM seguidores WHERE seguidor=$id   ORDER BY id DESC"
);
  } if( ($gender == "female")){
       $seleciona_produtos = mysql_query("SELECT * FROM tech WHERE categoria=2  && type=1  ORDER BY id DESC LIMIT $inicio, $maximo"
); 
...
<div style=""> <center><a class="hidden-xs hidden-sm"style="  text-decoration:none;color:#fff" href="tech.php?pag=1"><center><div style=" margin-left:45%; margin-top:-13px; width:100px; height:40px; font:#03D1F5; border:solid 2px #CCC; background-color:#fff; " class=" img-rounded col-sm-12 col-xs-12  " ><h4 style=" color:#03D1F5">More</h4></div></center></a></center>
					<a class="col-sm-12 col-xs-12 hidden-lg hidden-md" style="  text-decoration:none;color:#fff" href="tech.php?pag=1"><div style=" float:right;width:100%; height:40px; font:#fff; border:solid 2px #CCC; background-color:#09f; " class=" img-rounded  " ><center><h4>More</h4></center></div></a> 
