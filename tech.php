...
<section id="new"><div class="container"><a style="text-decoration:none;" href="recent.php"><h3 style=" font-family:Verdana, Geneva, sans-serif; color:#0CF;">&nbsp;</h3></a><BR>
  ...
  $inicio = ($pag * $maximo) - $maximo;
    ...
    if( ($gender == "male")){
      $seleciona_produtos = mysql_query("SELECT * FROM tech WHERE categoria=1 or categoria=0  ORDER BY id DESC  LIMIT $inicio, $maximo"
); 
$seleciona_seguindo = mysql_query("SELECT * FROM seguidores WHERE seguidor=$id   ORDER BY id DESC"
);
  } if( ($gender == "female")){
       $seleciona_produtos = mysql_query("SELECT * FROM tech WHERE categoria=2 or categoria=0  ORDER BY id DESC LIMIT $inicio, $maximo"
); 
...
$seleciona_seguindo = mysql_query("SELECT * FROM seguidores WHERE seguidor=$id   ORDER BY id DESC"
  ...
  if(mysql_num_rows($seleciona_produtos) == 0){
		  ?>
          <BR><br><br><br><br>
          <center><h1 style="color:#F03">SOON..</h1></center><br><br><br><br><br><br>
	 <?php }else{
			 ...
			 for($i = $pag-$links; $i <= $pag-1; $i++){
			  ...
			  for($i = $pag+1; $i <= $pag+1; $i++){
				if($i>$paginas){
					...
					echo'<a class="hidden-xs hidden-sm"style=" float:right; text-decoration:none;color:#fff" href="tech.php?pag='.$i.'"><div style=" float:right;width:100px; height:40px; font:#fff; border:solid 2px #CCC; background-color:#09f; " class=" img-rounded col-sm-12 col-xs-12  " ><h4>More</h4></div></a>
					<a class="col-sm-12 col-xs-12 hidden-lg hidden-md" style="  text-decoration:none;color:#fff" href="tech.php?pag='.$i.'"><div style=" float:right;width:100%; height:40px; font:#fff; border:solid 2px #CCC; background-color:#09f; " class=" img-rounded  " ><center><h4>More</h4></center></div></a> 
					';
